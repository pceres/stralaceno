...
$debug_mode = (array_key_exists("debug",$_REQUEST) && ($_REQUEST['debug'] === "full"));
...
foreach ($elenco_giocate2 as $indice_giocata => $giocata)
{
	...
	foreach($gruppo_risposta as $indice_risposta => $gruppo)
	{
		...
		if (!array_key_exists($risposta,$punteggio_specifico[$indice_risposta])) // non e' indicato il punteggio...
		{
			$punti = '-';
...
// 	echo "$data_giocata -> {$temp[0]},{$estremi_date[2]},{$estremi_date[3]}<br>\n";
	...
	//
	// Definizione dei simboli e legenda da utilizzare, per ciascun gruppo e ciascun punteggio
	//
	// Ci saranno due array ($simbolo_ok e $simbolo_not_ok), una per le risposte ok, l'altra per quelle non ok.
	// Essi sono costituiti da un array per ciascun gruppo, es. quarti (4), semifinali (3), finale (2), vincitore (1).
	// Ciascun sottoarray è costituito da un array (simbolo,legenda) per ciascun punteggio assegnato (es. 4,3,2,1,0).
	// Se si lascia la legenda vuota, il simbolo non verrà visualizzato nella legenda
	...
	$sub_array_ok_G = Array(
			...
			'6' 		=> Array('<small>6G</small>'		,'','Qualificazione tra le migliori terze classificate (6 punti)'),
	...
	$sub_array_not_ok_G = Array(
			...
			'3' 		=> Array('mg'		,'','Eliminazione per effetto della sconfitta nello scontro diretto o per effetto della peggiore differenza reti o classifica avulsa (6 punti da ripartire su 2 squadre)'),
	...
	foreach ($list0 as $id)
	{
		...
		if (in_array($vettore_giocata[$id],$risposte_equivalenti[$gruppo]))
		{
			...
			$simbolo = $item_simbolo[0];		// simbolo
			$stile_simbolo = $item_simbolo[1];	// stile
			$legend_simbolo = $item_simbolo[2];	// descrizione per la legenda
		...
		else
		{
			...
			$tipo_simbolo = 'not_ok';
			$simbolo_item = $simbolo_not_ok[$gruppo];
			if ($punti !== '-') // se e' stato definito un punteggio (anche nullo)...
			{
				$item_simbolo = $simbolo_item[$punti];
...
// 				$punti_msg = 'il match relativo non e' stato ancora disputato';
				$punti_msg = '0 punti';
			}
		...
		$indice_simbolo = $simbolo.'_'.$legend_simbolo;
		if (!empty($legend_simbolo) && !in_array($indice_simbolo,$elenco_simboli_usati) )
		{
...
&nbsp;&nbsp;
<?php
// inserisci legenda solo se ci sono almeno due colori diversi
if (count($lotteria['stili_riga']) > 1)
{
?>
Legenda colori:
<div class="txt_link">
...
if (count($elenco_simboli_usati)>0)
{
	foreach($elenco_simboli_item as $tipo_simbolo => $simboli_gruppo)
	{
		foreach($simboli_gruppo as $gruppo => $simboli_item)
		{
			foreach ($simboli_item as $punti => $item_simbolo)
			{
				...
				$chiave_simbolo = $simbolo_value.'_'.$simbolo_testo;
				if (in_array($chiave_simbolo,$elenco_simboli_usati))
				{
					$elenco_simboli_usati = array_diff($elenco_simboli_usati,Array($chiave_simbolo));
